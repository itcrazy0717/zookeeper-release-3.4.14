...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    synchronized public void startLeaderElection() {
        ...
        // getView返回server.id=ip:port:port，这里是初始化是进行的设置
        for (QuorumServer p : getView().values()) {
            // 获取当前zkserver myid对应的ip地址
            if (p.id == myid) {
                // 地址： 1 ->myQuorumAddr=192.168.13.102
                myQuorumAddr = p.addr;
                break;
            }
        }
        ...
        // electionType 匹配对应的选举算法
        // electionType 默认值为 3.可以在配置文件中动态配置 QuorumPeerConfig中的默认值
        if (electionType == 0) {
            try {
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm) {
        ...
        switch (electionAlgorithm) {
            ...
            // 默认选举策略为3    
            case 3:
                // 跟选举有关系，用来接收投票的。QuorumCnxManager用来实现领导选举中的网络连接功能
    ...
    public void run() {
        ...
        try {
             ...
             */
            while (running) {
                // 第一次启动的时候，LOOKING 默认就是LOOKING状态
                switch (getPeerState()) {
                    // 竞选状态
                    case LOOKING:
                        LOG.info("LOOKING");
                        ...
                        if (Boolean.getBoolean("readonlymode.enabled")) {
                            ...
                            try {
                                setBCVote(null);
                                // 设置当前选票，通过策略模式来决定当前用什么选举算法，默认使用FastLeaderElection
                                setCurrentVote(makeLEStrategy().lookForLeader());
                            } catch (Exception e) {
                                LOG.warn("Unexpected exception", e);
                                setPeerState(ServerState.LOOKING);
                    ...
                    case FOLLOWING:
                        try {
                            LOG.info("FOLLOWING");
                            // 构建一个FollowerZooKeeperServer，表示follower节点的请求处理服务
